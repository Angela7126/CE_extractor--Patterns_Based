...
#---------- Paragraph class ----------
class Parag:
    def __init__(self, c, p, num, slist):
        self.Ctag = c
        self.Ptag = p
        self.sent_num = num
        self.slist = slist
...
#######################################################################################
def FilePaser():
    [file_pn,file_pn_new] = Get_Build_fpathes(TXTcorpdir,TXTpkdir) # file_pn is the file path set of all files in TXTcorpdir, and file_pn_new is the new file path set in TXTpkdir according to each file_pn  
    filenum = len(file_pn) #filenum is the number of files in the corpus
    Flist = [] # Flist is the paper class list for every paper in file_pn
    ...
    RIDpattern = re.compile(r'(?<=\[)[\d]+(?=\])') # regular expression pattern to get RID
    parser = stanford.StanfordParser(model_path="E:\jars\englishPCFG.ser.gz") #Setting stanford-parser 
    ...
    for fi in range(filenum):
        ...
        for i in range(4,len(P_txt_list)):
            ...
            # if digit like "1.6 asdfkl" has shown, the following paragraph should put into a subsection
                ...
                tempSC_list.append(SubSec(CID,SCID,SCtitle[1]+' '+' '.join(S_txt_list[1:Snum]))) #create a new empty subsection object
            ...
            # if the current P_txt_list[i] is the text
                ...
                if Curstate == 3: # if has find "Reference", put the following reference into Curfile.Refer
                    RID = int((RIDpattern.search(S_txt_list[0])).group())
                ...
                tempP_list.append(Parag(CID,PID,SID-Sstart,tempS_list[Sstart:SID])) # create Parag object
    ...
    Dumppickle(os.path.join(os.getcwd(),"PK","DIC","PaperList.pk"), Flist);
    return Flist
...
#         cmyShowFileKeyW(f)
    #    WriteXls(f)     
    Flist = Loadpickle(os.path.join(DICpkdir,'PaperList.pk'));
    for f in Flist:
        print f.Ftag, f.Ftitle;
    #cmyShowFileInfo(f)
    #cmyShowFileKeyW(f)
    #cmyShowFileAbs(f)
    #cmyShowFileRefer(f)

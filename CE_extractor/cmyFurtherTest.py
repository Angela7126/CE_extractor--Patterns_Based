...
def CESecDistribution2TXT(Recfp,PaperTestList):
    ...
    for papertest in PaperTestList:
        ...
        for CESec in papertest.CESeclst:
            ...
            RecordFile.write("\tCovered_Rate: %.4f" %(float(len(CESec.CEsent))*100/len(CESec.Slist) if len(CESec.Slist)>0 else 0) + "%\n\n");
        RecordFile.write("#-------------------------------------------------\n\n");
    ...
    for papertest in PaperTestList:
        ...
        for CESec in papertest.CESeclst:
            ...
            for idx,ce in enumerate(CESec.CElist):
                ...
                RecordFile.write("\n");
                RecordFile.write("\t\tPattern: "+'%d' %ce.pt.pfreq + ' %s' %ce.pt.main_token+"----"+ ' %s' %ce.pt.constraints +'\n');
...
def GetPaperTest():
    PaperList = Loadpickle(os.path.join(DICpkdir,"PaperList.pk"));
    CEList = Loadpickle(os.path.join(DICpkdir,"CEList.pk"));
    PaperTestList = [];
    ConcRegExp = re.compile(ur'(?:conclusion|summary)', re.I); # Regular Expression using to check whether a section is the 'conclusion'
    ...
    Dumppickle(os.path.join(DICpkdir,"PaperTestList_FurtherText.pk"), PaperTestList)    
...
#     PaperTestList = GetPaperTest();
    PaperTestList = Loadpickle(os.path.join(DICpkdir,"PaperTestList_FurtherText.pk"));
    Recfp = os.path.join(ctxtdir,'CESec_Distribution_FurtherTest.txt');
    CESecDistribution2TXT(Recfp,PaperTestList);
#     FCoWord = [];
#     for papertest in PaperTestList:
#         FCoWord.append(papertest.CE_AC_CoWord);
#     CoWordfp = os.path.join(ctxtdir,'CE_AC_CoWordsPercent_FurtherTest.txt');
